...
namespace LittleGrayCalculator.Cores
{
    ...
    internal class BigCalculate
    {
        ...
        public static BigNumber Multiply(BigNumber one, BigNumber two) {
            ...
            BigNumber result = new BigNumber(intPart, decimalPart, !(one.IsPlus ^ two.IsPlus));
            return result;
        }

        /// <summary>除</summary>
        public static BigNumber Division(BigNumber one, BigNumber two) {
            return Division(one, two, one.DecimalPart.Count > two.DecimalPart.Count ? one.DecimalPart.Count : two.DecimalPart.Count);
        }
        ...
        public static BigNumber Modular(BigNumber one, BigNumber two)
        {
            int minusInt = 0;
            List<int> oneText = one.IntPart;
            List<int> twoText = two.IntPart;
            List<int> intPart;
            List<int> decimalPart;

            decimalPart = DecimalMinus(one.DecimalPart, two.DecimalPart, ref minusInt);

            intPart = Modular(oneText, twoText);
            return new BigNumber(intPart, decimalPart, true);
             
        }
        /*
        internal static List<int> Modular(List<int> one, List<int> two)
        {
            if (two == null || two.Count == 0 || (two.Count == 1 && two[0] == 0))
            {
                throw new ArgumentException("Divisor (two) cannot be null or zero.");
            }

            RemoveStartZero(one);
            RemoveStartZero(two);
            //  
            List<int> result = new List<int>(one);

             
            // Perform long division until `result` is less than `two`
            while (CompareList(result, two) >= 0)
            {
                // Subtract `two` from `result`
                result = SubtractLists(result, two);
            }
        ...
        */
        ...
        private static int BASE = BigNumber.Max; // 每个元素最多 9 位，基数为 10^9

        public static List<int> Modular(List<int> one, List<int> two)
        {
            if (two == null || two.Count == 0 || (two.Count == 1 && two[0] == 0))
            {
                throw new ExpressionException("余数不能为0或NULL");
            }
            ...
            // 如果 `two` 是单一小数值，可以直接对每个高位操作
            if (two.Count == 1 && two[0] < BASE)
            {
                long mod = 0;
                foreach (var digit in one)
                {
                    mod = (mod * BASE + digit) % two[0];
                }
                return new List<int> { (int)mod };
            }

            // `result` 用于存储当前剩余
            List<int> result = new List<int>(one);

            // 长除法：当 `result >= two` 时进行减法
            while (CompareLists(result, two) >= 0)
            {
                // 获取当前的高位差值，并进行快速缩减
                int shift = result.Count - two.Count;
                List<int> scaledTwo = ScaleAndShift(two, shift);

                // 如果缩放后的数比当前余数大，调整缩放
                if (CompareLists(result, scaledTwo) < 0)
                {
                    shift--;
                    scaledTwo = ScaleAndShift(two, shift);
                }
                if (result.Count < 5)
                {
                    ;
                ...
                // 减去缩放后的 `two`
                result = SubtractLists(result, scaledTwo);
            }
        ...
        private static List<int> SubtractLists(List<int> a, List<int> b)
        {
            ...
            for (int i = 0; i < b.Count || borrow != 0; i++)
            {
                int ai = result[result.Count - 1 - i];
                int bi = (i < b.Count) ? b[b.Count - 1 - i] : 0;
        ...
        private static void RemoveLeadingZeros(List<int> num)
        {
        ...
        private static List<int> ScaleAndShift(List<int> num, int shift, int baseValue)
        {
            var scaled = new List<int>(new int[shift]); // 在低位添加 `shift` 个 0
            scaled.AddRange(num);
            return scaled;
        }


        private static List<int> ScaleAndShift(List<int> num, int shift)
        {
            if (shift < 0) return new List<int>(num); // 不需要缩放
            List<int> scaled = new List<int>(num);
        ...
        internal static List<int> IntMinus(List<int> one, List<int> two, int Minus) {
            ...
            List<int> result = new List<int>(one);
            int borrow = 0;
            for (int i = 0; i < two.Count || borrow != 0; i++)
            {
                int ai = result[result.Count - 1 - i];
                int bi = (i < two.Count) ? two[two.Count - 1 - i] : 0;
            ...
            */
            ...
            for (int i = 0; i < two.Count; i++) {
                ...
                if (oneResult < 0) {
                    ...
                    if (result.Count - 2 - i > -1)
                    {
                        result[result.Count - 2 - i]--;
                        if (result[result.Count - 2 - i] < 0)
                        {
                            result.RemoveAt(result.Count - 2 - i);
                        }
