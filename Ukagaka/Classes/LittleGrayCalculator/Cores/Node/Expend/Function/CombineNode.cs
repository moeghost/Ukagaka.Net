...
namespace LittleGrayCalculator.Cores
{
    ...
    class CombineNode : FunctionNode
    {
        ...
        public override BigNumber Value {
            get {
                if (Nexts.Count >= 2)
                {
                    ...
                    { 
                        throw new ExpressionException("Comb的参数只能是正整数", Nexts[1].Index, Nexts[1].Format.Length);
                    }
                }
                return Statistics.Combine(Nexts[0].Value, Nexts[1].Value);
            }
        }
